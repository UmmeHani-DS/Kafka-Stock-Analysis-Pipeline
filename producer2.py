...
url=f'https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=AAPL&apikey=VMANK1SUI8B7VTY4'
while True:
    response = requests.get(url)
    data = response.json()
    if 'Global Quote' in data:
         message=data['Global Quote']
         producer.send("STOCK_OF_APPLE",value =message)
         print(f"PRODUCER   SENT APPLE STOCK  {data['Global Quote']}")
    time.sleep(20)     
